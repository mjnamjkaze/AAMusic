...
abstract class RenameApkTask : DefaultTask() {
    ...
    fun run() {
        ...
        inDir.listFiles()?.filter { it.extension == "apk" }?.forEach { f ->
            ...
            // Also copy to project root directory
            val rootDest = File(rootDir, newName)
            f.copyTo(rootDest, overwrite = true)
            println("APK copied to: ${rootDest.absolutePath}")
        }
    }
}
